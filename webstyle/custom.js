// AT THE MOMENT THIS FILE IS NOT INCLUDED
...
/***** Customer Filter ******/
        ...
        var navbar = document.getElementById("customers-buttons");
        var sticky = navbar.offsetTop;
        
        $(window).scroll(function(){
            if (window.pageYOffset >= sticky) {
                $(navbar).addClass($("#story-buttons .active").attr('id') + ("-bg"));
                navbar.classList.add("sticky");                
            } else {
                navbar.classList.remove("sticky");
                $(navbar).removeClass($(navbar).attr('class').split(' ').pop()).addClass("kashmir-blue-bg");
            } 
        });

        $("#story-buttons button").click(function() {  
            if($(navbar).hasClass("sticky") && !$(navbar).hasClass(this.id + ("-bg")) && $(this.id) != "all") {
                    $(navbar).removeClass($("#story-buttons .active").attr('id') + ("-bg"))
                    $(this).addClass("active");
                    $(navbar).addClass(this.id + ("-bg"));
                    $("#story-buttons button").not(this).removeClass("active");
            }
            if (this.id !== "all") {
                $(".story-filter").removeClass("alt-bg");
                $(".cs-bottom").css("background-color","#fff");
            }
            if (this.id == "all") {
                $(".story-filter").addClass("alt-bg");
                $(".cs-bottom").css("background-color","#f4f7f9");
            }
        });
        
        var $btns = $('.customer-btn').click(function() {
            if (this.id == 'all') {
                $('#customer-stories > div').fadeIn(450);
            } else {
                var $el = $('.' + this.id).fadeIn(450);
                $('#customer-stories > div').not($el).hide();
            }
